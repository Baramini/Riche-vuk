...
class BasicLightingPass : public IRenderPass {
 public:
  ...
  VkSampler GetColourSampler() const { return m_linearSampler; }
  VkImageView GetColourImageView(uint32_t idx) const { return m_colourBufferImages[idx].imageView; }

  VkSampler GetShadowSampler() const { return m_linearSampler; }
  VkImageView GetShadowImageView(uint32_t idx) const { return m_raytracingImages[idx].imageView; }
 ...
 private:
  ...
  void CreateMeshShaderPipelineLayout();
